...
describe("insight mapper", () => {
  it("normalizes concatenated multi-source summaries into a task summary", () => {
  ...
  it("keeps a real synthesized summary even when it is detailed", () => {
    const summary = toInsightSummary({
      id: "insight-2",
      aiInsight: "Ethereum 雷达：基金会治理调整",
      aiSummary:
        "多家来源共同显示，Ethereum 相关讨论正在从单点新闻转向基金会治理、资产配置和交易所流动性三条线索。这个组合更像是叙事关注度的扩散，而不是单一事件本身。",
      type: "NEWS",
      sentiment: "NEUTRAL",
      heatScore: 61,
      heatVelocity: 19,
    ...
    expect(summary.ai_summary).toContain("叙事关注度的扩散");
    expect(summary.ai_summary).not.toContain("2 个来源");
  });
});
