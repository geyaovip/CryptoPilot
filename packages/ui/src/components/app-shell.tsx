...
export function AppShell({ title, navItems, children, className, renderSidebarFooter, variant = "default" }: AppShellProps) {
  ...
  return (
    <div className={cn("min-h-screen", isPerplexity ? "bg-[#FCFCF9] text-[#102A2C]" : "bg-white text-slate-950")}>
      ...
      {renderSidebarFooter ? (
        <div className="fixed bottom-20 left-4 z-40 w-56 md:hidden">{renderSidebarFooter()}</div>
      ) : null}
      <nav
        className={cn(
          "fixed inset-x-0 bottom-0 grid grid-cols-4 border-t md:hidden",
          isPerplexity ? "border-[#D9D5C9] bg-[#FCFCF9]/95 backdrop-blur" : "border-slate-200 bg-white"
        )}
      >
        {navItems.slice(0, 4).map((item) => (
          <a
            className={cn("px-2 py-3 text-center text-xs", isPerplexity ? "text-[#5F6868]" : "text-slate-600")}
            href={item.href}
            key={item.href}
          >
            {item.label}
